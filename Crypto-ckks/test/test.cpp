...
using namespace std;
 ...
 int main()
 {
   ...
   //  std::cin>>a>>b;
    FHE_CKKS CKKS;
    CKKS.Init();
    std::string m="/home/demoData/cipherAddLocation11.txt";
    double rsads=CKKS.Decrypt(m);
    std::cout<<rsads<<endl;
    // double c=1;
    // double d=2;
    // std::string Encrypt1=CKKS.Encrypt((double)c);
    // std::string Encrypt2=CKKS.Encrypt((double)d);
