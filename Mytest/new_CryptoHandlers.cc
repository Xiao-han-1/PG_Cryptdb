...
/*=====================  SERIALIZE Helpers =============================*/
std::string StrFromval(uint64 val)
{
     std::stringstream _p;
     p<<val;
     std::string str;
...
Items 
RND_int::decrypt(Items  const ctext, uint64_t IV) const
{
    const uint64_t c = varFromStr(ctext.context);
    const uint64_t p = bf.decrypt(c) ^ IV;
    LOG(encl) << "RND_int decrypt " << c << " IV " << IV << " --> " << p;
    Items re=new_item(StrFromvar(p),ctext.type,StrFromvar(p).length);
    return re;
}
...
Items *
RND_str::encrypt(const Items &ptext, uint64_t IV) const
{
    const std::string enc =
        encrypt_AES_CBC(ptext.content, enckey,
...
Items
RND_str::decrypt(Items const ctext, uint64_t IV) const
{
    const std::string dec =
        decrypt_AES_CBC(ctext.context, deckey,
                        BytesFromInt(IV, SALT_LEN_BYTES), false);
    LOG(encl) << "RND_str decrypt " << ItemToString(*ctext) << " IV "
              << IV << "-->" << "len of dec " << dec.length()
              << " dec: " << dec;
    Items re=new_item(dec,ctext.type,dec.length);
    return re;
}
...
Items new_item(std::string context,enum_pg_types type,uint len)
{
    Items it;
    it.context=context;
    it.type=type;
...
Items 
DET_str::decrypt(Items  const ctext, uint64_t IV) const
{
    const std::string enc = ctext.context;
...
ZZToItemInt(const ZZ &val)
{
    ...
    Items re=new_item(StrFromvar(v),BIGINT,StrFromvar(v).length);
    return re;
...
{
    const std::string dec = StringFromZZ(val);
    Items re=new_item(dec,ctext.type,dec.length);
    return re;
}

static ZZ
ItemStrToZZ(Items const i)
{
    const std::string res = i.context;
...
Items
HOM::decrypt(Items const ctext, uint64_t IV) const
{
    if (true == waiting) {
        this->unwait();
    }
...
Items 
Search::decrypt(Items const ctext, uint64_t IV) const
{
