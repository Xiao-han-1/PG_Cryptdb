                       ...
                       std::unique_ptr<RecoveryDetails> *details)
{
    ...
    assert(PQntuples(dbres->n) == 1);

    // const MYSQL_ROW embedded_row = mysql_fetch_row(dbres->n);
    // const unsigned long *const l = mysql_fetch_lengths(dbres->n);
    const std::string string_embedded_begin(PQgetvalue(dbres->n,0,0));
    const std::string string_embedded_complete(PQgetvalue(dbres->n,1,1));
    const std::string string_embedded_query(PQgetvalue(dbres->n,2,2));
    const std::string string_embedded_default_db(PQgetvalue(dbres->n,3,3));
    ...
    const unsigned long remote_row_count = PQntuples(dbres->n);
    std::string string_remote_begin, string_remote_complete;
    const bool existed_remote = !!remote_row_count;
    if (existed_remote) {
        // const unsigned long *const l = mysql_fetch_lengths(dbres->n);
        string_remote_begin    = std::string(PQgetvalue(dbres->n,0,0));
        string_remote_complete = std::string(PQgetvalue(dbres->n,1,1));
                 ...
                 const std::unique_ptr<Connect> &e_conn)
{
    ...
    const unsigned long long unfinished_count = PQntuples(dbres->n);
    ...
    // const unsigned long *const l = mysql_fetch_lengths(dbres->n);
    const std::string string_unfinished_id(PQgetvalue(dbres->n,0,0));
    const std::string string_unfinished_type(PQgetvalue(dbres->n,1,1));
