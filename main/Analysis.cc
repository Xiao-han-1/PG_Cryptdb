                           ...
                           std::string *const out_db)
{
    ...
    // assert(1 == PQntuples(db_res->n));
    int rows=PQntuples(db_res->n);
     for(int i=0;i<rows;i++)
     
    {
    out_db[i] = PQgetvalue(db_res->n,i,0);
    }
    if (out_db->size() == 0) {
                         ...
                         const std::unique_ptr<Connect> &e_conn)
{
    ...
    const std::string &q_completion =
        ...
        "    '" + TypeText<CompletionType>::toText(completion_type)
            + "') returning id;";
    std::unique_ptr<DBResult> res;
    e_conn->execute(q_completion,&res);
    int k_id=std::atoi(PQgetvalue(res->n,0,0));
    std::cout<<k_id<<"\n";
    // SYNC_IF_FALSE(e_conn->execute(q_completion), e_conn);
    this->embedded_completion_id = k_id;
    // SYNC_IF_FALSE(e_conn->execute(q_completion), e_conn);
...
{
    TEST_Sync(e_conn->execute("BEGIN TRANSACTION;"),
              "failed to start transaction");
    ...
    const
{
    ...
    const bool reissue = string_to_bool(PQgetvalue(db_res->n,0,0));
