                ...
                unsigned long long *const connection_id)
{
    ...
    const std::string &query = " SELECT pg_backend_pid();";
    RETURN_FALSE_IF_FALSE(c->execute(query, &dbres));
    RETURN_FALSE_IF_FALSE(PQntuples(dbres->n) == 1);

    // const MYSQL_ROW row = mysql_fetch_row(dbres->n);
    // const unsigned long *const l = mysql_fetch_lengths(dbres->n);
    *connection_id =
        strtoull(std::string(PQgetvalue(dbres->n,0,0)).c_str(), NULL, 10);
    return true;
}
                        ...
                        std::string *const out_name)
{
     ...
     *out_name = std::string(PQgetvalue(dbres->n,0,0));
    return true;
}
